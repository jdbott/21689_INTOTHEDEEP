...
@Config
@Autonomous(name = "Blue Pedro Cycle Test", group = "Pedro Auto")
public final class PedroAutoPathWallSide extends LinearOpMode {
    ...
    public void runOpMode() {
        ...
        Follower follower = new Follower(hardwareMap, true);
        ...
        Path purplePath = new Path(new BezierCurve(new Point(14.5, 62.0, Point.CARTESIAN), new Point(32.0, 23.0, Point.CARTESIAN)));
        ...
        sched.addAction(new SequentialAction(new DrivePoseLoggingAction(follower, "purple_path_begin", true), new FollowPathAction(follower, purplePath, false), new DrivePoseLoggingAction(follower, "purple_path_end"), new SleepAction(0.5)));
        sched.run();
...
//        yellowPath.setLinearHeadingInterpolation(currentPose.heading.toDouble(), Math.toRadians(180));
        ...
        follower.update();
        sched.addAction(new SequentialAction(new DrivePoseLoggingAction(follower, "yellow_path_begin"), new FollowPathAction(follower, yellowPath), new DrivePoseLoggingAction(follower, "yellow_path_end"), new SleepAction(1.25)));
        sched.run();
        ...
        while (cycleCount++ < 2) {
            ...
            Path toStack0 = new Path(new BezierLine(stagePoint, new Point(30, y_position0, Point.CARTESIAN)));
            toStack0.setConstantHeadingInterpolation(Math.PI);
            //toStack0.setZeroPowerAccelerationMultiplier(3.75);

            sched.addAction(new SequentialAction(new DrivePoseLoggingAction(follower, "stack0_path_begin"), new FollowPathAction(follower, toStack0), new DrivePoseLoggingAction(follower, "stack0_path_end")));

            sched.run();

            Path toStack1 = new Path(new BezierCurve(new Point(follower.getPose()), new Point(0, y_position0, Point.CARTESIAN), new Point(-16, y_position0, Point.CARTESIAN)));

            //toStack1.setZeroPowerAccelerationMultiplier(3.5);
            toStack1.setConstantHeadingInterpolation(Math.PI);

            Path toStack2 = new Path(new BezierCurve(new Point(-16, y_position0, Point.CARTESIAN), new Point(-38, y_position0, Point.CARTESIAN), new Point(-45, y_position0, Point.CARTESIAN), new Point(-52, y_position, Point.CARTESIAN)));
            ...
            sched.addAction(new SequentialAction(new DrivePoseLoggingAction(follower, "stack_path_begin"), new FollowPathAction(follower, builder.build()), new DrivePoseLoggingAction(follower, "stack_path_end"), new SleepAction(1.75)));
            sched.run();

            follower.update();
            Pose2d stack0 = follower.getPose();
            Path toStage0 = new Path(new BezierLine(new Point(stack0), new Point(-40, y_position0, Point.CARTESIAN)));
            ...
            sched.addAction(new SequentialAction(new DrivePoseLoggingAction(follower, "stage0_path_begin"), new FollowPathAction(follower, toStage0), new DrivePoseLoggingAction(follower, "stage0_path_end")));
            sched.run();

            follower.update();
            Path toStage1 = new Path(new BezierCurve(new Point(follower.getPose()), new Point(8, y_position0, Point.CARTESIAN), new Point(32, y_position0, Point.CARTESIAN), cycle));
            ...
            // drop yellow
            sched.addAction(new SequentialAction(new DrivePoseLoggingAction(follower, "stage1_path_begin"), new FollowPathAction(follower, toStage1), new DrivePoseLoggingAction(follower, "stage1_path_end"), new SleepAction(1.75), new DrivePoseLoggingAction(follower, "cycle_end")));
            sched.run();
        }
